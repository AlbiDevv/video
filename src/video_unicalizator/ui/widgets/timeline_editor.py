...
class TimelineEditorWidget(ctk.CTkFrame):
    """Таймлайн в стиле монтажки с thumbnail-strip и waveform."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._pending_thumbnail_key: str | None = None
        ...
        self.info_label = ctk.CTkLabel(
            header,
            text="Double click создаёт клип. Drag двигает, drag края меняет длину, Delete удаляет.",
            text_color="#94a3b8",
            justify="left",
        )
        ...
        self.add_music_button.grid(row=0, column=5, padx=(0, 8))

        self.playhead_label = ctk.CTkLabel(controls, text="00:00.0 / 00:00.0", text_color="#dbe4f0")
        self.playhead_label.grid(row=0, column=6, sticky="e")
    ...
    def set_interaction_enabled(self, enabled: bool) -> None:
        ...
        for widget in (self.zoom_out_button, self.zoom_in_button, self.fit_button, self.playhead_button, self.end_button, self.add_music_button):
    ...
    def _track_center_y(self, lane: TimelineLane) -> float:
        return self._lane_top(lane) + self.TRACK_HEIGHT / 2
    ...
    def _content_width(self) -> int:
        ...
        return max(
            visible_width,
            int(self._timeline_left() + self._duration * self._pixels_per_second + trailing_room),
        )
    ...
    def _redraw_tracks_only(self) -> None:
        ...
        total_height = int(self._lane_origin_top() + len(self.LANE_META) * (self.TRACK_HEIGHT + self.TRACK_GAP) + 18)
    ...
    def _draw_thumbnail_strip(self, full_width: int) -> None:
        ...
        thumb_size = (84, self.THUMBNAIL_HEIGHT - 8)
        thumbs = self._thumbnail_cache.peek(self._video_path, count=10, size=thumb_size)
        if not thumbs:
            self._request_thumbnail_strip(size=thumb_size)
            self.canvas.create_text(
                left + 12,
                top + self.THUMBNAIL_HEIGHT / 2,
                text="Thumbnail strip появится после загрузки видео.",
                fill="#64748b",
                anchor="w",
                font=("Segoe UI", 10),
            )
        ...
        segment_width = (right - left) / len(thumbs)
        for index, thumb in enumerate(thumbs):
            image = ImageTk.PhotoImage(thumb, master=self.canvas)
            self._thumbnail_refs.append(image)
            center_x = left + segment_width * index + segment_width / 2
            self.canvas.create_image(center_x, top + self.THUMBNAIL_HEIGHT / 2, image=image)
        ...
        for lane, title, color in self.LANE_META:
            ...
            if lane == "Music" and not lane_clips:
                self.canvas.create_text(
                    ...
                    text="Double click или кнопка + Music создают clip на текущем playhead.",
                    fill="#8ea2c0",
                    anchor="w",
                    font=("Segoe UI", 10),
                    tags=("tracks",),
    ...
    def _request_thumbnail_strip(self, *, size: tuple[int, int]) -> None:
        key = self._thumbnail_cache.key_for(self._video_path, count=10, size=size)
        if key is None or key == self._pending_thumbnail_key:
            return
        self._pending_thumbnail_key = key
        ...
        self._thumbnail_cache.request_async(self._video_path, count=10, size=size, callback=on_ready)

    def _on_thumbnail_ready(self) -> None:
        self._pending_thumbnail_key = None
        if self.winfo_exists():
            self._redraw()
